...
/**
 * @fileoverview Cura WASM Printer REsolver Tests
 */

//Imports
import {expect} from 'chai';
import {resolvePrinter} from '../src/index';

describe('resolve printers', () =>
{
  it('will not resolve primary printers', () =>
  {
    const printer1 = resolvePrinter('fdmextruder');
    const printer2 = resolvePrinter('fdmprinter');

    expect(printer1).to.null;
    expect(printer2).to.null;
  });

  it('will resolve single-dependency printers', () =>
  {
    const printer = resolvePrinter('ultimaker');

    expect(printer).to.eql({
      version: 2,
      name: 'Ultimaker',
      inherits: 'fdmprinter',
      metadata: {
  ...
  it('will resolve multi-dependency printers', () =>
  {
    const printer = resolvePrinter('ultimaker2');
